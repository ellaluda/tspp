...
void execute_jacobi(double *curr_grid, double *next_grid, int rows, int columns, int rank, int total_procs) {
    ...
    for (count = 0; count < ITERATION_LIMIT; count++) {
        ...
        // 边界元素交换
        if (rank > 0) {
            MPI_Isend(curr_grid + columns, columns, MPI_DOUBLE, rank - 1, 0, MPI_COMM_WORLD, &send_request[num_requests]);
            MPI_Irecv(curr_grid, columns, MPI_DOUBLE, rank - 1, 0, MPI_COMM_WORLD, &recv_request[num_requests]);
            num_requests++;
        }

        if (rank < total_procs - 1) {
            MPI_Isend(curr_grid + rows * columns, columns, MPI_DOUBLE, rank + 1, 0, MPI_COMM_WORLD, &send_request[num_requests]);
        ...
        // 停堆检查
        if (count % 100 == 0 && delta < EPSILON) break;
    }
