...
void parallel_merge_sort(vector<int>& A, vector<int>& B, int left, int right, int depth) {
    ...
    if (depth > 0) {
        ...
        parallel_merge_sort(A, B, left, middle, depth - 1);

        #pragma omp task shared(A, B) if (depth > 0)
        parallel_merge_sort(A, B, middle, right, depth - 1);
...
int main(int argc, char* argv[]) {
    ...
    cout << "Arr " << (is_equal ? "equal" : "diffrent") << endl;
